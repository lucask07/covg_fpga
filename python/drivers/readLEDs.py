# Duplicated from the Ad5453 board in GitHub
import sys
...
def to_voltage(a):
    bits = 16 # ADS8686 is a 16 bit ADC   
    vref = 2.5*2 
    masked = a & 0x600 # page 62... resets CHANNEL_SEL    
    v = masked/(2**bits - 1)*vref
    chan = (a & 0xf000) >> 12 # leave alone?
    return v, chan
...
//
//  *** are these the same for the ADS8686?? ***
// 
//  2'b00   Read 
...
if __name__ == "__main__":
    ...
    # MUX control (0 picks ADS8686)
    ads8686 = 0
    dacs = [1,2,3,4]
    f.set_wire(0x01, ads8686, mask = 0xffffffff)

    # SPI Master configuration: divide reg, ctrl reg, SS register 
    # MSB: 8 - set address, 4 - write data  

    ## check this! ##
    creg_val = 0x40003610 # Char length of 16; set both Tx_NEG, Rx_NEG; set ASS, IE. ADS8686
    ...
    if mode == 'manual':
        ...
        # TODO: 
        # 1) setup software mode by sending wire in commands to the wishbone master
        ...
        # setup 
        for val in [0x80000001, 0x40008000,  # enter software program sequence
                    0x80000041, creg_val | (1 << 8),
                    0x80000001, 0x4000000f,  # channels 0,1,2,3   
                    0x80000041, creg_val | (1 << 8),
                    0x80000001, 0x40002840,  # enter software mode
                    0x80000041, creg_val | (1 << 8)]: # Control register - GO (bit 8)
