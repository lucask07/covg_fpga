...
def configure_clamp(fpga, ADC_SEL=None, DAC_SEL=None, CCOMP=None, RF1=None,
    ...
    DAC_SEL_code = CAL_ADC_dict.get(DAC_SEL)
    CCOMP_code = CCOMP_dict.get(CCOMP)
    RF1_code = RF1_dict.get(RF1)

    # I/O Expander 2 (io2)
    ADG_RES_code = ADG_RES_dict.get(ADG_RES)
    PClamp_CTRL_code = PClamp_CTRL
    P1_E_CTRL_code = P1_E_CTRL
    P1_CAL_CTRL_code = P1_CAL_CTRL
    P2_E_CTRL_code = P2_E_CTRL
    P2_CAL_CTRL_code = P2_CAL_CTRL
    gain_code = gain_dict.get(gain)
    FDBK_code = FDBK
    mode_code = mode_dict.get(mode)
    EN_ipump_code = EN_ipump
    RF_1_Out_code = RF_1_Out
...
CAL_ADC_dict = {#Choose to drive/store CAL_Sig1 and CAL_Sig2
    'drive_CAL1' : 0b0111,
    'drive_CAL2' : 0b1011,
    'gnd_CAL2' : 0b1101,
    'gnd_CAL1' : 0b1110,
