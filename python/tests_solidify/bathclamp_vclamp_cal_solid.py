...
class Headstage:
    def __init__(self, device_compilation : DeviceSetUp):
        self.device_compilation = device_compilation

                # Set the dac channel
        self.adc_chan0 = self.device_compilation.fpga_board.daq.parameters['ads_map'][dc_under_test]['CAL_ADC']  # ('A', 0)
        self.adc_chan1 = self.device_compilation.fpga_board.daq.parameters['ads_map'][dc_under_test]['AMP_OUT']  # ('A', 1)
        self.adc_v1 = self.device_compilation.fpga_board.daq.parameters['ads_map'][device_compilation.fpga_board.dc_mapping['clamp']]['AMP_OUT']  # ('A', 1)
    ...
    def dac_waveform(self, dc_under_test, amp, freq=1000, shape='SINE', source='v', periods=None):
        ...
        for i in range(6):
            ...
            self.device_compilation.fpga_board.ddr.data_arrays[i][:] = 0x2000
        ...
        if shape == 'SINE':
            sdac_wave, freq = self.device_compilation.fpga_board.ddr.make_sine_wave(amplitude=sdac_amp_code,
            ...
            sdac_wave = self.device_compilation.fpga_board.ddr.make_step(low=int(dac80508_offset - sdac_amp_code),
            ...
            if sdac_amp_code > dac80508_offset:
                print(f'error, square-wave amplitude of {sdac_amp_code} too large ')
            sdac_wave, freq, indices = self.device_compilation.fpga_board.ddr.make_chirp(amplitude=sdac_amp_code,
                                                    frequencies=freq, periods=periods,
                                                    offset=dac80508_offset)

        # Specify output channel for DAC80508
        sdac_ch = self.device_compilation.fpga_board.daq.parameters['gp_dac_map'][dc_under_test]['CAL']
        if sdac_ch[0] == 1:
        ...
        for i in range(6):
            self.device_compilation.fpga_board.ddr.data_arrays[i] = np.bitwise_and(
                self.device_compilation.fpga_board.ddr.data_arrays[i], 0x3fff
            )
        # Load data into DDR
        # Set channel bits
        self.device_compilation.fpga_board.ddr.data_arrays[0] = np.bitwise_or(
            self.device_compilation.fpga_board.ddr.data_arrays[0], (sdac_1_out_chan & 0b110) << 13
        )
        self.device_compilation.fpga_board.ddr.data_arrays[1] = np.bitwise_or(
            self.device_compilation.fpga_board.ddr.data_arrays[1], (sdac_1_out_chan & 0b001) << 14
        )
        self.device_compilation.fpga_board.ddr.data_arrays[2] = np.bitwise_or(
            self.device_compilation.fpga_board.ddr.data_arrays[2], (sdac_2_out_chan & 0b110) << 13
        )
        self.device_compilation.fpga_board.ddr.data_arrays[3] = np.bitwise_or(
            self.device_compilation.fpga_board.ddr.data_arrays[3], (sdac_2_out_chan & 0b001) << 14
        )

        # load slow DAC sine-wave in DDR channels 6
        self.device_compilation.fpga_board.ddr.data_arrays[6] = sdac_wave
        # ddr.data_arrays[7] = sdac_sine # TODO: all cal channels are on the first DAC so setting [7] is not needed

        self.device_compilation.fpga_board.ddr.write_setup()
        block_pipe_return, speed_MBs = self.device_compilation.fpga_board.ddr.write_channels(set_ddr_read=False)
        self.device_compilation.fpga_board.ddr.reset_mig_interface()
        self.device_compilation.fpga_board.ddr.write_finish()
    ...
    def get_ads_voltages(self, ddr, dc_configs, num_repeats=1, blk_multiples=10):
        ...
        sys_connections = create_sys_connections(
            dc_configs, 
            self.device_compilation.fpga_board.daq, 
            self.device_compilation.instrument.ephys_sys
        )
    ...
    def setup_clamps(self, dc_under_test, dc_disconnect):
        ...
        dc_configs = {}
        log_info_test, dc_configs[dc_under_test] = self.device_compilation.fpga_board.clamps[dc_under_test].configure_clamp(
            ADC_SEL='CAL_SIG2',
            DAC_SEL="drive_CAL2",
            CCOMP=cap,
            RF1=fb_res,  # feedback circuit
            ADG_RES=res,
            PClamp_CTRL=0, # keep open for calibration (default) 0 1 1 1 1
        ...
        log_info_disconnect,  dc_configs[dc_disconnect] = self.device_compilation.fpga_board.clamps[dc_disconnect].configure_clamp(
            ADC_SEL='CAL_SIG1', # since the electrodes are disconnect there will be no calibration signal  
            DAC_SEL="noDrive",
            CCOMP=cap,
            RF1=fb_res,  # feedback circuit
            ADG_RES=res,
            PClamp_CTRL=0,  # open relay (default) 0 1 0 1 0
            P1_E_CTRL=1,  # open relay
        ...
        log_info, dc_configs[self.device_compilation.fpga_board.dc_mapping['guard']] = self.device_compilation.fpga_board.clamps[self.device_compilation.fpga_board.dc_mapping['guard']].configure_clamp(
            ...
            RF1=fb_res,  # feedback circuit
            ADG_RES=res,
            PClamp_CTRL=0, # we can also close the clamp control to trap the current within the circle only
            P1_E_CTRL=1,
            P1_CAL_CTRL=0,
            P2_E_CTRL=1,
            P2_CAL_CTRL=0,
        ...
        log_info_vclamp, dc_configs[self.device_compilation.fpga_board.dc_mapping['vclamp']] = self.device_compilation.fpga_board.clamps[self.device_compilation.fpga_board.dc_mapping['vclamp']].configure_clamp(
            ADC_SEL="CAL_SIG1",
            DAC_SEL="gnd_both",
            CCOMP=cap,
            RF1=fb_res,  # feedback circuit
            ADG_RES=res,
            PClamp_CTRL=1,  # open relay (default)
            P1_E_CTRL=1,  # open relay
            P1_CAL_CTRL=1,  # open relay (default)
            P2_E_CTRL=1,  # open relay
            P2_CAL_CTRL=1,  # open relay (default)
            gain=1,  # instrumentation amplifier
            FDBK=1,
            mode="voltage",
            EN_ipump=0,
        ...
        dc_configs[self.device_compilation.fpga_board.dc_mapping['vclamp']]['VSENSE'] = 60.24 # extra information for the system_connections. Gain of the voltage clamp amplifier # TODO: uncomment when done
        # dc_configs[dc_mapping['clamp']]['VSENSE'] = 60.24*10**(-11.7868/20) 
        sys_connections = create_sys_connections(dc_configs, self.device_compilation.fpga_board.daq, self.device_compilation.instrument.ephys_sys)
    ...
    def measure_resistance(self, config_dict_test, dc_configs, dc_under_test, testing='bath', step=1, 
        ...
        self.device_compilation.fpga_board.daq.set_isel(port=1, channels=[dc_under_test]) # current based on DC#  
        ...
        log_info_bath, config_dict_test = self.device_compilation.fpga_board.clamps[dc_under_test].configure_clamp(**config_dict_test)
        ...
        volt, t, ads_separate_data, fig_collect, ax = self.collect_data(
            self.device_compilation.fpga_board.ddr, PLT=plt_data, 
            ads_chan=self.device_compilation.fpga_board.daq.parameters['ads_map'][dc_under_test]['CAL_ADC'],
            num_repeats=num_repeats, 
            blk_multiples=blk_multiples
        )

        figures_table[f"Collect data for {testing}"] = fig_collect
        ...
        figures_table[f"Resistance plot for {testing}"] = fig
        ...
        rdata[step] = {'volt': volt,
                    ...
                    'amp': current_amp,
                    'vclamp': 'disconnect',
                    'bath_clamp': copy.deepcopy(dc_configs[self.device_compilation.fpga_board.dc_mapping['bath']]),
                    'voltage_clamp': copy.deepcopy(dc_configs[self.device_compilation.fpga_board.dc_mapping['clamp']]),
                    'vsense': copy.deepcopy(dc_configs[self.device_compilation.fpga_board.dc_mapping['vclamp']])}
        try:
        ...
        dc_status = {self.device_compilation.fpga_board.dc_mapping.inverse[dc] : copy.deepcopy({k : dc_configs[dc][k] for k in ['ADC_SEL', 
                                                                        'DAC_SEL',  
                                                                        'PClamp_CTRL', 
                                                                        'P1_E_CTRL', 
                                                                        'P2_E_CTRL', 
                                                                        'P1_CAL_CTRL', 
                                                                        'P2_CAL_CTRL']}) for dc in dc_configs}
        self.EXPERIMENTS.update({f"measure resistance of {testing}" : 
                            [figures_table, 
                            dc_status, 
    ...
    def chirp_test(self, testing, data_chirp, dc_configs, dc_under_test, voltage_amp, step_chirp, 
        ...
        for drive_elec in [1,2]:
            ...
            # dc_configs[dc_disconnect]['DAC_SEL'] = 'noDrive'
            # dc_configs[dc_disconnect]['P1_CAL_CTRL'] = 0.0
            # dc_configs[dc_disconnect]['P2_CAL_CTRL'] = 0.0
            ### Compile the new configs ###

            log_info_bath, dc_configs[dc_under_test] = self.device_compilation.fpga_board.clamps[dc_under_test].configure_clamp(**dc_configs[dc_under_test])
            ...
            volt_chirp, t_chirp, ads_separate_data_chirp, fig, ax = self.collect_data(self.device_compilation.fpga_board.ddr, PLT=False,
                                                                            ads_chan=
                                                                            self.device_compilation.fpga_board.daq.parameters['ads_map'][dc_under_test][
                                                                                'CAL_ADC'],
            ...
            relays_daqs[f'drive_elec_{drive_elec}'] = {self.device_compilation.fpga_board.dc_mapping.inverse[dc] : copy.deepcopy({k : dc_configs[dc][k] for k in ['ADC_SEL', 
                                                                        'DAC_SEL',  
                                                                        'PClamp_CTRL', 
                                                                        'P1_E_CTRL', 
                                                                        'P2_E_CTRL', 
                                                                        ...
                                                                        'P2_CAL_CTRL']}) for dc in dc_configs}
            param_list.extend([
                relays_daqs[f'drive_elec_{drive_elec}'], 
                dict()
            ])
            ...
            for freq, idx in zip(freq_arr, indices):
                ...
                if dc_under_test==self.device_compilation.fpga_board.dc_mapping['bath']:
                    v1_gain = 1
                else:
            ...
            relays_daqs[f'drive_elec_{drive_elec}_swap'] = {self.device_compilation.fpga_board.dc_mapping.inverse[dc] : copy.deepcopy({k : dc_configs[dc][k] for k in ['ADC_SEL', 
                                                                        'DAC_SEL',  
                                                                        'PClamp_CTRL', 
                                                                        'P1_E_CTRL', 
                                                                        'P2_E_CTRL', 
            ...
            param_list_swap.extend([
                relays_daqs[f'drive_elec_{drive_elec}_swap'], 
                dict()
            ])
    ...
    def chirp_test_vclamp(self, testing, data_chirp, dc_configs, dc_under_test, voltage_amp, step_chirp, 
        ...
        for float_dut in [True, False]:
            ...
            """
            if float_dut:
                dc_configs[dc_disconnect]['P1_E_CTRL'] = 1 # open
                dc_configs[dc_disconnect]['P2_E_CTRL'] = 1 # open
                dc_configs[dc_disconnect]['P1_CAL_CTRL'] = 0 # open
                dc_configs[dc_disconnect]['P2_CAL_CTRL'] = 0 # open 
            else:
                dc_configs[dc_disconnect]['P1_E_CTRL'] = 1 # open
                dc_configs[dc_disconnect]['P2_E_CTRL'] = 1 # open
                dc_configs[dc_disconnect]['P1_CAL_CTRL'] = 1 # open
                dc_configs[dc_disconnect]['P2_CAL_CTRL'] = 1 # open 
            dc_configs[dc_disconnect]['DAC_SEL'] = 'gnd_both' 
            """

            # Deal with the guard
            dc_guard = self.device_compilation.fpga_board.dc_mapping['guard']
            dc_configs[dc_guard]['DAC_SEL'] = 'gnd_both'
            ...
            dc_configs[dc_under_test]['ADC_SEL'] = 'CAL_SIG1'
            dc_configs[dc_under_test]['DAC_SEL'] = 'drive_CAL2' # do not ground CAL1 
            dc_configs[dc_under_test]['PClamp_CTRL'] = 1.0


            ### Compile the new configs ###
            log_info_dut, dc_configs[dc_under_test] = self.device_compilation.fpga_board.clamps[dc_under_test].configure_clamp(**dc_configs[dc_under_test])
            # log_info_load, dc_configs[dc_disconnect] = self.device_compilation.fpga_board.clamps[dc_disconnect].configure_clamp(**dc_configs[dc_disconnect])
            _, dc_configs[dc_guard] = self.device_compilation.fpga_board.clamps[dc_guard].configure_clamp(**dc_configs[dc_guard])
            ...
            relays_daqs[f'float_dut_{float_dut}'] = {self.device_compilation.fpga_board.dc_mapping.inverse[dc] : copy.deepcopy({k : dc_configs[dc][k] for k in ['ADC_SEL', 
                                                                        'DAC_SEL',  
                                                                        'PClamp_CTRL', 
                                                                        'P1_E_CTRL', 
                                                                        'P2_E_CTRL', 
    ...
    def transfer_functions_fit(self, testing, data_chirp, dc_configs, dc_under_test, voltage_amp, step_chirp, freq_arr, dc_disconnect, 
                            r_total_guess):
        measured_params = None
        if testing == 'bath':
            data_chirp, measured_params, filename_chirp, step_chirp = self.chirp_test(testing, data_chirp, dc_configs, dc_under_test, 
                                                                    voltage_amp, step_chirp, freq_arr=freq_arr, dc_disconnect=dc_disconnect)
        elif testing=='clamp':
            data_chirp, measured_params, filename_chirp, step_chirp = self.chirp_test_vclamp(testing, data_chirp, dc_configs, dc_under_test, 
                                                                voltage_amp, step_chirp, freq_arr, dc_disconnect)
        # process data
        if testing == 'clamp':
            tf_type = 'vclamp'
            r_total_guess = 300e3
        ...
        predicted_res, res_fit_mesg, component_fits, fit_notes, components, figures_table = total_res_iso_res(data_chirp,  
...
#############################################################
def experiment(device_compilation : DeviceSetUp) -> Headstage:
    headstage = Headstage(device_compilation)
    headstage.device_compilation.fpga_board.ddr.data_version = 'TIMESTAMPS'

    headstage.device_compilation.fpga_board.configure_dac_80508()

    headstage.device_compilation.fpga_board.daq.set_isel(port=1, channels=None)
    headstage.device_compilation.fpga_board.daq.set_isel(port=2, channels=None)
    ...
    setup_info = {
        ...
        'board': 2, 'rej1': 200e3, 
        'rpcj1':10e3, 'srj1': 1e3, 
        'dc_mapping': dict(headstage.device_compilation.fpga_board.dc_mapping), 
    ...
    for i in range(6):
        # set all fast-DAC DDR data to midscale
        headstage.device_compilation.fpga_board.ddr.data_arrays[i][:] = 0x2000
    ...
    # Data for the 2 DAC80508 "Slow DACs"
    sdac_sine, sdac_freq = headstage.device_compilation.fpga_board.ddr.make_sine_wave(amplitude=sdac_amp_code, frequency=target_freq_sdac, offset=dac80508_offset)

    # Specify output channel for DAC80508
    sdac_ch = headstage.device_compilation.fpga_board.daq.parameters['gp_dac_map'][0]['CAL']
    if sdac_ch[0] == 1:
        sdac_1_out_chan = sdac_ch[1]
    if sdac_ch[0] == 2:
        sdac_2_out_chan = sdac_ch[1]  # don't care, this channel is not connected to CAL
    ...
    # Clear bits in the FDAC DDR stream that store the slow DAC channel
    for i in range(6):
        headstage.device_compilation.fpga_board.ddr.data_arrays[i] = np.bitwise_and(
            headstage.device_compilation.fpga_board.ddr.data_arrays[i], 0x3fff
        )

    # Load data into DDR
    # Set channel bits # TODO: make this a method of the DDR
    headstage.device_compilation.fpga_board.ddr.data_arrays[0] = np.bitwise_or(
        headstage.device_compilation.fpga_board.ddr.data_arrays[0], (sdac_1_out_chan & 0b110) << 13
    )
    headstage.device_compilation.fpga_board.ddr.data_arrays[1] = np.bitwise_or(
        headstage.device_compilation.fpga_board.ddr.data_arrays[1], (sdac_1_out_chan & 0b001) << 14
    )
    headstage.device_compilation.fpga_board.ddr.data_arrays[2] = np.bitwise_or(
        headstage.device_compilation.fpga_board.ddr.data_arrays[2], (sdac_2_out_chan & 0b110) << 13
    )
    headstage.device_compilation.fpga_board.ddr.data_arrays[3] = np.bitwise_or(
        headstage.device_compilation.fpga_board.ddr.data_arrays[3], (sdac_2_out_chan & 0b001) << 14
    )

    # load slow DAC sine-wave in DDR channels 6
    headstage.device_compilation.fpga_board.ddr.data_arrays[6] = sdac_sine  # TODO: just one
    headstage.device_compilation.fpga_board.ddr.data_arrays[7] = sdac_sine
    ...
    for testing in ['bath', 'clamp']:
        ...
        data_chirp = {}
        if testing == 'bath':
            dc_under_test = headstage.device_compilation.fpga_board.dc_mapping['bath']  # TODO: get these indices from the boards configuration
            dc_disconnect = headstage.device_compilation.fpga_board.dc_mapping['clamp']
        elif testing == 'clamp':
            dc_under_test = headstage.device_compilation.fpga_board.dc_mapping['clamp']
            dc_disconnect = headstage.device_compilation.fpga_board.dc_mapping['bath']

        dc_configs, sys_connections = headstage.setup_clamps(dc_under_test=dc_under_test, dc_disconnect=dc_disconnect)

        headstage.device_compilation.fpga_board.ddr.write_setup()
        block_pipe_return, speed_MBs = headstage.device_compilation.fpga_board.ddr.write_channels(
            set_ddr_read=False)  # TODO: is this actually used or just promptly overwritten?
        headstage.device_compilation.fpga_board.ddr.reset_mig_interface()
        headstage.device_compilation.fpga_board.ddr.write_finish()
        ...
        # log_info_bath, dc_configs[dc_disconnect] = headstage.device_compilation.fpga_board.clamps[dc_disconnect].configure_clamp(**dc_configs[dc_disconnect])

        # disable the current source 
        headstage.device_compilation.fpga_board.daq.set_isel(port=1, channels=None) # channel select works correctly -- this turns off the signal 
        # ---- CHIRP testing ------------
