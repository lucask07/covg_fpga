...
for data_key in data:
    ...
    if d['shape'] == 'SQ':
        ...
        yfit = curve_fit(soft_sq_wave, t,y , p0=(freq, 0.016, 0, 0, 0)) # freq, amp, offset, phase, smoothing factor 
                        #bounds = ([0,0,-15,0,0], [10e6, np.inf, 15, 2*np.pi, np.inf]))  # bounds cause problems. Not sure why
        predicted_res = yfit[0][1]/current_amp
...
for (drive_elec, meas_adc) in [('drive_CAL1', 'CAL_SIG1'), ('drive_CAL2', 'CAL_SIG2')]:
    ...
    ax[1].semilogx(fit_results[drive_elec]['freq'], -np.degrees(fit_results[drive_elec]['phase']), 
        marker='*', color=clr)
...
if tf_type == 'elec_r_cc':
    r1 = component_fits['drive_CAL1'][idx].params['r1'].value
    r2 = component_fits['drive_CAL2'][idx].params['r1'].value
    r3 = np.average([component_fits['drive_CAL1'][idx].params['r3'].value, 
