...
try:  # only initialize systeam and FPGA if needed. Allows us to repeat with %run -i tests/bathclamp_vclamp_step_response.py
    ...
    time.sleep(0.05)
    ad7961s[
        0].reset_trig()  # this IS required because it resets the timing generator of the ADS8686. Make sure to configure the ADS8686 before this reset
...
def setup_clamps(dc_under_test, dc_disconnect):
    ...
    log_info, dc_configs[dc_mapping['guard']] = clamps[dc_mapping['guard']].configure_clamp(
    ...
    dc_configs[dc_mapping['vclamp']]['VSENSE'] = 60.24 # extra information for the system_connections. Gain of the voltage clamp amplifier # TODO: uncomment when done
...
def measure_resistance(config_dict_test, dc_configs, dc_under_test, testing='bath', step=1, 
    ...
    dc_configs[dc_mapping['clamp' if testing=='bath' else 'bath']]['ADC_SEL'] = 'CAL_SIG1'

    # TODO: How is the disconnected clamp board configured? 
    if testing == 'bath':
        config_dict_test['DAC_SEL'] = 'drive_CAL2_gnd_CAL1'
        dc_configs[dc_mapping['clamp']]['DAC_SEL'] = 'drive_CAL1'
    # elif testing == 'vlcamp':
    elif testing == "clamp":
        config_dict_test['DAC_SEL'] = 'drive_CAL2'
        dc_configs[dc_mapping['bath']]['DAC_SEL'] = 'drive_CAL1'
...
# Helper functions to configure DACs (quiet, enable DDR, etc.)
def config_dacs(config='host', ch=[None, None]):
    '''
    configure the FPGA data mux (default to host driven mode)
    and write the offset value 
    for both the fast_dac and the slow_dac
...
def quiet_dacs(ch=[None, None]):
    # calls config dacs in order to stop continuous SPI writes to DACS
...
if 0:
    ...
    dc_configs[dc_mapping['bath']], rdata, fit_resistance, pcov, mesg, ads_separate_data = measure_resistance(
        dc_configs[dc_mapping['bath']],
        dc_under_test=dc_mapping['bath'],
        testing='bath', step=1, plt_data=True,
        plt_fit=True, write_ddr=True)
...
if 0:
    ...
    dc_configs[dc_mapping['clamp']], rdata, fit_resistance, pcov, mesg, ads_separate_data = measure_resistance(
...
if 0:
    ...
    clamp_dac_val = electrode_offset_cal(clamp_num=dc_num, chan=daq.parameters['ads_map'][dc_num]['CAL_ADC'],
