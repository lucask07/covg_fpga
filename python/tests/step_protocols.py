...
protocol = Protocol.create_from_csv(filepath=r'ephys/dagan_comparison.csv', num_sweeps=7)
protocol.preview()
...
if 1:
    ...
    TO_CLAMPFIT = True
    if TO_CLAMPFIT: # TODO 
        datastreams.to_clampfit(data_dir, 'step_protocols_boxclosed_rtia{}_ccomp{}.abf'.format(res, cap),
                        names_pclamp = ['Im', 'CMD0', 'V1', 'P1'],
                        dac_len=len(protocol), dac_sample_rate=2.5e6, sweeps=len(protocol.sweeps))

    ds_equal = datastreams.equal_datastream(names_pclamp=['Im', 'CMD0', 'V1', 'P1'], 
                                            dac_sample_rate=2.5e6, 
                                            dac_len=len(protocol))
