...
```json
{
    ...
    "ADS8686_LPF" : 376,
    "ADS8686_SEQUENCER_SETUP" : [["0", "0"], ["1", "1"], ["4", "2"]],
...
```
Pay attention to the data type of each key (constant) in this JSON object. When users attempt to modify the value of any parameter listed in such an object, **_the new value must be of the exact same data type as the old value's_**.
...
UID MANUFACTURER_CODE FAILED
    (read) 163 != (default) 41
```
A side note is users may choose not to care about socket 2 because that is where the voltage sense daughtercard will be connected. Otherwise, all other sockets must be something other than 255.
...
```
with `<vsense_lowergain>` and `<vsense_uppergain>` are two gain values for the voltage sense's TCA, whose details can be found in the `Vsense2` class in the [`boards.py`](/python/boards.py) file.
...
```
To list all the keys in these dictionaries, it is recommended to run:

```py
>> headstage.EXPERIMENTS.keys()
[Out 1]: dict_keys(['measure resistance of bath', 'Transfer functions fit for bath', 'measure resistance of clamp', 'Transfer functions fit for clamp'])
>> # ... accordingly for the other two
```
...
It is also critical to know if the data pipelines in the DAC and TCA for all daughtercards are smooth and not disrupted at any cost. In other words, what was written to those should also be what they will read out. For example, the `Headstage` class also has a [`Vsense2`](/python/boards.py) member, allowing users to directly access and change its DAC and TCA writes for subsequent read tests:
```py
...
>> read_test.get_log()
[Out 2]: INFO:ReadTestLogger:Reads for Bath
        DAC 0 read = (100, 0)
        TCA 0 read = [0, 122]
        TCA 1 read = [0, 122]
Reads for Guard
...
#### Notes
In conclusion, check out the documentation of the [`HardwareSetup`](/python/tests_solidify/bathclamp_vclamp_utils.py), [`Headstage`](/python/tests_solidify/bath_clamp_headstage.py), and [`ReadTest`](/python/tests_solidify/DAC_TCA_reads_test.py) classes, together with the ones in the [`bathclamp_vclamp_utils.py`](/python/tests_solidify/bathclamp_vclamp_utils.py) and [`try_out_vsense_gains.py`](/python/tests_solidify/try_out_vsense_gains.py) files for more details!
